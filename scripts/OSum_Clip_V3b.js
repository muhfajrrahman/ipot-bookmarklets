  ...
  if (data.length) {
    ...
    navigator.clipboard.writeText(text)
      .then(() => {
        ...
        alert(`Data disalin ke clipboard, sebanyak ${data.length} baris`);
      })
      .catch(err => {
        console.warn("❌ Gagal salin ke clipboard:", err);
        alert("❌ Gagal salin ke clipboard!");
      });
  } else {
    alert("Tidak ada data ditemukan! Mungkin halaman belum memuat transaksi.");
  }
})();
